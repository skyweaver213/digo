 ...
 * - 特殊情况：如果通配符不含 /(末尾除外)，则两者意义相同：都表示匹配任意层目录。比如 `*.js` 既匹配 `foo.js` 也匹配 `usr/foo.js`。如果希望只匹配根目录下的 .js 文件，应该写成 `/*.js`。
 *
 * ##### 正则表达式
 * 复杂的匹配规则可以使用正则表达式。
...
function globToRegExp(pattern: string, cwd: string) {
    const compiledPattern = compiledPatterns[pattern];
    if (compiledPattern && compiledPattern.cwd === cwd) {
        return compiledPattern;
    }

    // 判断是否存在 /。
    const slash = pattern.indexOf('/');
    const hasSlash = slash >= 0 && slash < pattern.length - 1;
    ...
    // 生成正则表达式。
    const result = compiledPatterns[pattern] = new RegExp(regex, np.sep === "\\" ? "i" : "") as any as typeof compiledPatterns[""];
    result.cwd = cwd;
    result.base = root;
    return result;
}
