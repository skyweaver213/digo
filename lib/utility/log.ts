...
export function formatLog(value: string, args?: Object) {
    return value.replace(/\{(?:([a-z]+):)?([^}]+)\}/g, (all, color: string, key: string) => `\u001b[${color && typeof ConsoleColor[color] === "number" ? ConsoleColor[color] : "1"}m${args != null && key in args ? args[key] : key}\u001b[39m`);
}
...
export function ellipsisLog(value: string, width = 0) {
    ...
    if (width <= 0) width += (process.stdout as WriteStream).columns || 80;
...
export function splitLog(value: string, width = 0) {
    ...
    for (let i = 0; i < value.length;) {
        ...
        // 忽略单词后的空格。
        while (i < value.length && value.charCodeAt(i) === 32/* */) i++;
        leftBound = left = i;
        currentWidth = 0;
    }
 ...
 */
export function formatSource(source: string, width = 0, height = 3, showLine = true, showColumn = true, startLine?: number, startColumn?: number, endLine?: number, endColumn?: number) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        let currentWidth = 0;
        for (let j = firstColumn; j <= line.length; j++) {
        ...
        // 生成行指示器。
        if (showColumn && lineNumber === startLine && columnMarkerStart != undefined) {
            result += "\n";
            if (showLine) {
