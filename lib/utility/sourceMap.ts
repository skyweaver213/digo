...
export function toSourceMapObject(sourceMapData: SourceMapData) {
    ...
    const sourceMapObject: SourceMapObject | IndexMapObject = typeof sourceMapData === "string" ?
        ...
        (sourceMapData as SourceMapGenerator).toJSON ?
            (sourceMapData as SourceMapGenerator).toJSON() :
            sourceMapData;
    if ((sourceMapObject as IndexMapObject).sections) {
    ...
    return sourceMapObject as SourceMapObject;
}
...
export class SourceMapBuilder implements SourceMapGenerator {
     ...
     */
    getSource(generatedLine: number, generatedColumn: number) {
     ...
     */
    getGenerated(sourcePath: string, sourceLine: number, sourceColumn: number) {
            ...
            if (mappings) {
                for (let i = 0; i < mappings.length; i++) {
                    ...
                    if (mapping.sourceIndex === expectedSourceIndex) {
                        if (mapping.sourceLine != undefined && mapping.sourceColumn != undefined) {
                                ...
                                for (const targetMapping of other.mappings[mapping.sourceLine]) {
                                    if (targetMapping.generatedColumn > mapping.sourceColumn) {
                                        ...
                                        const m: Mapping = {
                                            ...
                                            sourceIndex: this.addSource(targetMapping.sourceIndex == undefined ? undefined : other.sources[targetMapping.sourceIndex]),
                            ...
                            mapping.sourceIndex = this.addSource(other.file);
                        }
                    }
                }
            }
