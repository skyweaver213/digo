...
export class FileList {
        ...
        if (this.processor.collect) {
            ...
            if (file.generated) {
                ...
                for (index = 0; index < this.files.length; index++) {
                    if (this.files[index].srcPath === file.srcPath) {
                        break;
                    }
                }
            }
            ...
            if (this.processor.end) {
                if (this.processor.collect) {
                    ...
                    if (this.processor.end.length <= 3) {
                        (this.processor.end as Function)(this.files, this.processorOptions, this.next);
                        onEnd();
                    } else {
                        this.processor.end(this.files, this.processorOptions, this.next, onEnd);
                    }
                } else {
                    if (this.processor.end.length <= 3) {
                        (this.processor.end as Function)(undefined, this.processorOptions, this.next);
                        this._onEnd();
                    } else {
                        this.processor.end(undefined!, this.processorOptions, this.next, () => {
                            this._onEnd();
                        });
                    }
                }
    ...
    then(callback: (done: () => void) => void) {
        return this.pipe({
            collect: false,
            end: callback.length ? (files, options, dest, done) => {
                callback(done);
            } : () => {
