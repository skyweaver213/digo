...
import { resolvePath, relativePath, getDir, setDir, getExt, setExt, pathEquals } from "../utility/path";
import { resolveUrl, relativeUrl } from "../utility/url";
import { stringToBuffer, bufferToString, base64Uri } from "../utility/encode";
import { readFile, writeFile, copyFile, deleteFile, deleteParentDirIfEmpty, existsFile, getStat } from "../utility/fs";
...
export class File {
    ...
    /**
     * 获取当前文件的源绝对路径。如果当前文件是动态创建的则返回 undefined。
     */
     ...
     * 设置当前文件的绝对路径。
     */
    set path(value) { this.name = value == undefined ? undefined : relativePath(this.base || "", value); }

    /**
     * 获取当前文件的源文件夹绝对路径。
     */
    get srcDir() { return this.srcPath == undefined ? undefined : getDir(this.srcPath); }

    /**
     * 获取当前文件的目标文件夹绝对路径。
     */
    get destDir() { return this.destPath == undefined ? undefined : getDir(this.destPath); }
     ...
     * 设置当前文件的文件夹绝对路径。
     */
    set dir(value) { this.path = value == undefined ? undefined : setDir(this.path || resolvePath(".file"), value); }

    /**
     * 获取当前文件的源扩展名。
     */
    get srcExt() { return this.srcPath == undefined ? undefined : getExt(this.srcPath); }

    /**
     * 获取当前文件的目标扩展名。
     */
    get destExt() { return this.ext; }
     ...
     * 判断当前文件是否是生成的。
     */
    get generated() { return this.srcPath == undefined; }

    /**
     * 判断当前文件是否实际存在。
     */
    get exists() { return this.generated && existsFile(this.srcPath!); }

    /**
     * 获取当前文件的字符串形式。
     */
    toString() { return this.generated ? format("(Generated)") + (this.destPath == undefined ? "" : getDisplayName(this.destPath)) : getDisplayName(this.srcPath!); }

    /**
     * 提供直接查看当前文件对象的方法。
     */
    private inspect() { return "File " + this.toString(); }
    ...
    get srcBuffer() {
        if (this._srcBuffer == undefined) {
            if (this._srcContent == undefined) {
                if (this.srcPath && this.buildMode !== BuildMode.clean) {
                    const taskId = begin("Read: {file}", { file: this.toString() });
                    try {
                        this._srcBuffer = readFile(this.srcPath);
                    } catch (e) {
     ...
     * @return 返回对应的行列号。
     */
    indexToLocation(index: number) { return indexToLocation(this.content, index, this as any); }
     ...
     * @return 返回对应的索引。
     */
    locationToIndex(location: Location) { return locationToIndex(this.content, location, this as any); }
     ...
     * @desc 仅当 sourceMapEmit 为 true 时有效。
     */
    get sourceMapUrl() { return sourceMapUrl ? sourceMapUrl(this) : relativePath(this.dir || "", this.sourceMapPath); }
            ...
            for (let i = 0; i < sourceMapObject.sources.length; i++) {
                result.sourcesContent[i] = sourceMapSourceContent ?
                    ...
                    sourceMapObject.sourcesContent ?
                        ...
                        (sourceMapObject.sources[i] === this.srcPath ? this.srcContent : bufferToString(readFile(sourceMapObject.sources[i]), typeof encoding === "function" ? encoding(new File(sourceMapObject.sources[i])) : encoding));
            }
        }
    ...
    save(dir?: string, callback?: (error: NodeJS.ErrnoException | null, file: File) => void) {
        ...
        // 更新基路径。
        if (dir != undefined) this.base = resolvePath(dir);
    ...
    clone() {
        const result: File = {
            ...
            sourceMapData: this.sourceMapObject
        } as any;
        for (const key in this as File) {
            if (this.hasOwnProperty(key) && !result.hasOwnProperty(key)) {
                let value = this[key];
     ...
     * 获取当前文件的属性对象。
     */
    get stats() { return this.generated ? null : getStat(this.srcPath!); }
     ...
     */
    match(matcher: Pattern) { return this.path != undefined && new Matcher(matcher).test(this.path); }
     ...
     * @return 返回解析好的相对地址。
     */
    relative(url: string | File) { return relativeUrl(this.path || "", url instanceof File ? url.path || "" : url); }
...
export class FileLogEntry extends LogEntry {
    ...
    constructor(file: File, data?: string | Error | LogEntry, args?: Object) {
        ...
        // 统一文件对象。
        if (!this.file) {
            this.file = this.fileName != undefined && !pathEquals(this.fileName, file.srcPath!) ? new File(this.fileName, file.base) : file;
        }

        // 从文件提取信息。
        if (this.fileName == undefined) this.fileName = this.file.srcPath;
        ...
        if (evalSourceMap && this.sourceMap !== false && this.startLine != undefined && this.sourceMapData) {
            ...
            if (!pathEquals(this.fileName!, startSource.sourcePath)) {
                this.fileName = startSource.sourcePath;
                this.content = startSource.sourceContent;
                this.file = new File(this.fileName, file.base);
            }
            ...
            if (this.endLine != undefined) {
                ...
                if (pathEquals(this.fileName!, endSource.sourcePath)) {
        ...
        // 自动生成代码片段。
        if (this.content == undefined && this.source == undefined && this.startLine != undefined) {
            try {
