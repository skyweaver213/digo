...
export class LogEntry {
    ...
    toString(colors?: boolean) {
        ...
        // 添加堆栈信息。
        const errorStack = (logLevel === LogLevel.verbose || (this.showStack != undefined ? this.showStack : this.fileName == undefined || this.startLine == undefined && this.source == undefined)) && this.error && (this.error as Error).stack;
        if (errorStack) {
...
export function log(data: string | Error | LogEntry, args?: Object, level = LogLevel.log) {
    ...
    switch (level) {
        case LogLevel.error:
            ...
            if (colors) {
                prefix = addLogColor(prefix, ConsoleColor.red);
            }
        ...
        case LogLevel.warning:
            ...
            if (colors) {
                prefix = addLogColor(prefix, ConsoleColor.yellow);
            }
 ...
 */
export function format(message: string | null, args?: Object) {
    if (!message) {
        return "";
    }
